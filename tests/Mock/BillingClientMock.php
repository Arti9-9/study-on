...
class BillingClientMock extends BillingClient
{
    ...
    {
        /** @var UserDto $userAuthRequest */
        $userAuthRequest = $this->serializer->deserialize($request, UserDto::class, 'json');
        if (
            $userAuthRequest->username === $this->userDefault->username &&
            $userAuthRequest->password === $this->userDefault->password
        ) {
            $userAuthResponse = new UserDto();
            $userAuthResponse->token = $this->generateToken('ROLE_USER', $this->userDefault->username);
            $userAuthResponse->refreshToken = '999';
            return ($userAuthResponse);
        }

        if (
            $userAuthRequest->username === $this->userSuperAdmin->username &&
            $userAuthRequest->password === $this->userSuperAdmin->password
        ) {
            $userAuthResponse = new UserDto();
